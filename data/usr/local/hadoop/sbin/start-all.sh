...
if [ -f "${HADOOP_HDFS_HOME}"/sbin/mr-jobhistory-daemon.sh ]; then
  "${HADOOP_HDFS_HOME}"/sbin/mr-jobhistory-daemon.sh start historyserver
fi

# start zookeeper daemons if zookeeper is present
