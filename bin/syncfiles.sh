...
find /archlinux/data -maxdepth 1 -mindepth 1 -not -iname ".git" -not -iname ".ssh" | xargs --no-run-if-empty rm -r
...
cat /archlinux/config/backup_src.txt | xargs -I % bash -c "shopt -s globstar dotglob; cp --parents -R % ."
setfacl -m  u:aaron:rwx -R /archlinux
##cat /arch/config/backup_src.txt | xargs -I {} cp -- parents -R {} .
